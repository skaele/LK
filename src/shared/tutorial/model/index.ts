...
sample({
    ...
    filter: Boolean,
    target: [setRoles.prepend(() => []), checkHash],
})

sample({
    clock: checkHash.done,
    target: getTutorialData,
})

sample({
    clock: checkHash.fail,
    target: initialized.prepend(() => [...commonTutorialIds]),
})
sample({
    clock: $roles,
