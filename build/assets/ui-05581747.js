...
`,Ie=e=>`mutation RemoveStudentVisits {
  deleteStudentVisit(input: { historyId: ${e} }) {
    ...
    student(guid: "${e}") {
    ...
    }`,C=a(),ce=a(),B=i(0).on(ce,(e,t)=>t).on(y.$filters,()=>0),Q=q({source:{page:B,filters:y.$filters},effect:async({filters:e,page:t})=>{const{students:n}=await l(Be(t,ie(e)));return n}});re({source:O(B,y.$filters),timeout:200,target:C});const b=q({source:{filters:y.$filters},effect:async({filters:e})=>{const{students:t}=await l(He(ie(e)));return t.totalCount}});re({source:y.$filters,timeout:200,target:b});s({clock:C,target:Q});s({clock:C,target:b});const j=i([]).on(Q.doneData,(e,t)=>t.items),We=i(0).on(b.doneData,(e,t)=>t),Je=O(b.pending,Q.pending,Boolean);s({clock:F.addVisitFx.doneData,source:j,filter:(e,{studentGuid:t})=>e.some(n=>n.studentGuid===t),fn:(e,t)=>{const n=JSON.parse(JSON.stringify(e)),o=n.find(r=>r.studentGuid===t.studentGuid);return++o.visits,n},target:j});const ut={load:C,setPage:ce},lt={$loading:Je,$pEStudents:j,$pEStudentsPage:B,$pEStudentsTotalCount:We},Le=e=>`mutation addPointsToStudent{
        addPointsToStudent(
          input: ${V(e,["workType"])}
        ) {
          success
      ...
      }`,Ke=e=>`mutation removePointsFromStudent {
