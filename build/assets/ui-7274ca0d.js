import{p as i,e as u,x as s,h as l,v as T,m as z,cC as H,K as R,r as I,q as le,cG as me,cH as ge,j as F}from"./vendor-b80865ba.js";import{p as r,m as C,d,R as pe,E as fe,n as ve,o as ke,l as N,J as _}from"./index-5813d93c.js";const Se=({studentGuid:e,date:t})=>`mutation IncreaseStudentVisits {
...
}`,Ve=(e=null)=>{const t={...e,isActive:{eq:!0}};return`{
    students(where: ${P(t,["course"])}) {
      totalCount
    }
  }`},x=i({name:"load",sid:"-hr257w"}),Z=i({name:"setPage",sid:"34a6f9"}),b=l(0,{name:"$pEStudentsPage",sid:"9d5rx"}).on(Z,(e,t)=>t).on(p.$filters,()=>0),q=T({and:{source:{page:b,filters:p.$filters},effect:async({filters:e,page:t})=>{const{students:n}=await r(Re(t,Y(e)));return n}},or:{name:"loadPageFx",sid:"ckru1p"}});z({sid:"-mk8m28",fn:()=>H({source:R({and:[b,p.$filters],or:{name:"source",sid:"-mg50tn"}}),timeout:200,target:x}),name:"none",method:"debounce"});const A=T({and:{source:{filters:p.$filters},effect:async({filters:e})=>{const{students:t}=await r(Ve(Y(e)));return t.totalCount}},or:{name:"loadTotalCount",sid:"-fapsmo"}});z({sid:"-ln8ygk",fn:()=>H({source:p.$filters,timeout:200,target:A}),name:"none",method:"debounce"});s({and:[{clock:x,target:q}],or:{sid:"-lkhzhr"}});s({and:[{clock:x,target:A}],or:{sid:"-l6r4nq"}});const w=l([],{name:"$pEStudents",sid:"l1hwa7"}).on(q.doneData,(e,t)=>t.items),be=l(0,{name:"$pEStudentsTotalCount",sid:"dj5j8a"}).on(A.doneData,(e,t)=>t),qe=R({and:[A.pending,q.pending,Boolean],or:{name:"$loading",sid:"-11z05w"}});s({and:[{clock:y.addVisitFx.doneData,source:w,filter:(e,{studentGuid:t})=>e.some(n=>n.studentGuid===t),fn:(e,t)=>{const n=JSON.parse(JSON.stringify(e)),o=n.find(a=>a.studentGuid===t.studentGuid);return++o.visits,n},target:w}],or:{sid:"-krcw1m"}});const Ke={load:x,setPage:Z},Ye={$loading:qe,$pEStudents:w,$pEStudentsPage:b,$pEStudentsTotalCount:be},Ue=e=>`mutation addPointsToStudent{
        addPointsToStudent(
          input: ${P(e,["workType"])}
        ) {
          success
