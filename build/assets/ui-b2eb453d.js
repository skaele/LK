...
`,Ie=e=>`mutation RemoveStudentVisits {
  deleteStudentVisit(input: { historyId: ${e} }) {
    ...
    }`,V=a(),ce=a(),B=i(0).on(ce,(e,t)=>t).on(y.$filters,()=>0),H=q({source:{page:B,filters:y.$filters},effect:async({filters:e,page:t})=>{const{students:n}=await u(Be(t,ie(e)));return n}});re({source:O(B,y.$filters),timeout:200,target:V});const b=q({source:{filters:y.$filters},effect:async({filters:e})=>{const{students:t}=await u(Je(ie(e)));return t.totalCount}});re({source:y.$filters,timeout:200,target:b});s({clock:V,target:H});s({clock:V,target:b});const _=i([]).on(H.doneData,(e,t)=>t.items),Le=i(0).on(b.doneData,(e,t)=>t),Qe=O(b.pending,H.pending,Boolean);s({clock:h.addVisitFx.doneData,source:_,filter:(e,{studentGuid:t})=>e.some(n=>n.studentGuid===t),fn:(e,t)=>{const n=JSON.parse(JSON.stringify(e)),o=n.find(r=>r.studentGuid===t.studentGuid);return++o.visits,n},target:_});const ut={load:V,setPage:ce},lt={$loading:Qe,$pEStudents:_,$pEStudentsPage:B,$pEStudentsTotalCount:Le},We=e=>`mutation addPointsToStudent{
        addPointsToStudent(
          input: ${C(e,["workType"])}
        ) {
          success
