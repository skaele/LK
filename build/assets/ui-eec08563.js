...
`,Ie=e=>`mutation RemoveStudentVisits {
  deleteStudentVisit(input: { historyId: ${e} }) {
    ...
    }`,C=a(),ce=a(),B=i(0).on(ce,(e,t)=>t).on(y.$filters,()=>0),H=q({source:{page:B,filters:y.$filters},effect:async({filters:e,page:t})=>{const{students:n}=await l(Be(t,ie(e)));return n}});re({source:O(B,y.$filters),timeout:200,target:C});const b=q({source:{filters:y.$filters},effect:async({filters:e})=>{const{students:t}=await l(Je(ie(e)));return t.totalCount}});re({source:y.$filters,timeout:200,target:b});s({clock:C,target:H});s({clock:C,target:b});const _=i([]).on(H.doneData,(e,t)=>t.items),Qe=i(0).on(b.doneData,(e,t)=>t),Ze=O(b.pending,H.pending,Boolean);s({clock:F.addVisitFx.doneData,source:_,filter:(e,{studentGuid:t})=>e.some(n=>n.studentGuid===t),fn:(e,t)=>{const n=JSON.parse(JSON.stringify(e)),o=n.find(r=>r.studentGuid===t.studentGuid);return++o.visits,n},target:_});const ut={load:C,setPage:ce},lt={$loading:Ze,$pEStudents:_,$pEStudentsPage:B,$pEStudentsTotalCount:Qe},Le=e=>`mutation addPointsToStudent{
        addPointsToStudent(
          input: ${V(e,["workType"])}
        ) {
          success
