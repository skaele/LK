...
`,jt={[k.AdminAccess]:"Админ",[k.OnlineCourseAccess]:"Доступ к ЛМС",[k.SecretaryAccess]:"Секретарь",[k.DefaultAccess]:"Преподаватель"},yt=Object.entries(jt).map(([e,t])=>({title:t,id:e})),vt=()=>{const[e,t]=c.useState(""),[r,a]=c.useState(""),[i,o]=c.useState(null),u=()=>{je({fullName:e,teacherGuid:r,permission:i==null?void 0:i.id})};return s.jsxs(St,{children:[s.jsx(ft,{onChange:a,onSearchChange:t}),s.jsx(M,{title:"Доступ",items:yt,setSelected:o,selected:i}),s.jsx(m,{text:"Добавить",onClick:u})]})},W=l(),ye=l(),ve=h(async e=>{const{teachers:{items:t}}=await g(`
    query teachers {
      ...
      }`);return t}),Y=x("").on(W,(e,t)=>t);de({source:n({source:Y,filter:e=>e!==null}),timeout:200,target:ve});const kt=x([]).on(ve.doneData,(e,t)=>t);n({clock:ye,target:Y.reinit});const Ct=({onChange:e})=>{const[t,r]=S([kt,Y]),a=i=>{i!=null&&i.id&&e(i.id)};return c.useEffect(()=>(W(""),ye),[]),s.jsxs(s.Fragment,{children:[s.jsx(O,{size:5,align:"left",visible:!0,children:"Преподаватель"}),s.jsx(z,{value:r,setValue:W,hints:t.map(i=>({id:i.teacherGuid,title:i.fullName,value:i.teacherGuid})),onHintClick:a})]})},$t=e=>`mutation assignCuratorToGroup{
...
`,ke=l(),G=h(async e=>await g(Ot(e)));n({clock:ke,target:G});n({clock:G.doneData,target:y.close});n({clock:G.doneData,fn:()=>({message:"Новый семестр начат",type:"success",time:3e3}),target:p.evokePopUpMessage});n({clock:G.failData,fn:()=>({message:"Не удалось начать новый семестр",type:"failure",time:3e3}),target:p.evokePopUpMessage});const Mt={startNewSemester:ke},Tt=d.div`
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 10px;
...
`,Vt=()=>{const[e,t]=c.useState(""),r=()=>{Mt.startNewSemester(e)};return s.jsxs(Tt,{children:[s.jsx(P,{title:"Новый семестр",setValue:t,value:e}),s.jsx(m,{text:"Начать новый семестр",onClick:r})]})},_t=()=>`mutation {
      updateStudentsInfo {
        success
